...
void solve()
{
    ...
    vll prefix(n), sortedprefix(n);
    partial_sum(all(vec1), prefix.begin());
    partial_sum(all(vec2), sortedprefix.begin());
    ...
    while(q--){
        ...
        if(choice==1){
            ...
            if(l==1) cout<<sortedprefix[r-1]<<endl;
            else cout<<sortedprefix[r-1]-sortedprefix[l-2]<<endl;
        }
    }
}
