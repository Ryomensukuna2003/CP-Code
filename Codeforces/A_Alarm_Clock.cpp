...
void solve()
{
    ll need,limit,add_limit,buffer,count;cin>>need>>limit>>add_limit>>buffer;
    if(need<=limit){
        cout<<limit<<endl;
    }
    else if(need>limit && buffer>=add_limit){
        cout<<-1<<endl;
    }
    else{
        count=limit;
        need-=limit;
        count+=add_limit*(ceil((need*1.0)/(add_limit-buffer)));
        // while(need>0){
        //     count+=(add_limit);
        //     need-=(add_limit-buffer);
        // }
        cout<<count<<endl;
    }
}
