...
{
    int n,x;cin>>n>>x;
    vll vec1,prefix,suffix;
    forin(n,vec1);
    prefix.push_back(vec1[0]);
    suffix.push_back(vec1[n-1]);
    for(int i=1;i<n;i++){
        prefix.push_back(vec1[i]+prefix[i-1]);
    }
    for(int i=n-2;i>=0;i--){
        suffix.push_back(vec1[i]+suffix[n-i-2]);
    }
    reverse(all(prefix));
    reverse(all(suffix));
    for(int i=0;i<n;i++){
        if(prefix[i]%x!=0 || suffix[i]%x!=0){
            cout<<n-i<<endl;
            return;
        }
    }
