...
void solve()
{
    ...
    for(int i=0;i<n;i++){
        int temp1=(vec1[i].first-x);
        int temp2=(vec1[i].second-y);
        int abcd=abs(abs(temp1+temp2)-(i));
        cout<<x<<" "<<y<<"  "<<vec1[i].first<<" "<<vec1[i].second<<"  "<<abcd<<" "<<i+1<<endl; 
        if(i+1>abcd  && abcd%2==((abcd-(i+1))%2)){
            cout<<"Yes"<<endl;
            return;
        }
    }
